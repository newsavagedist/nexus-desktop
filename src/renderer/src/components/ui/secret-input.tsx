...
export function SecretInput({ className, value, ...props }: SecretInputProps) {
  ...
  return (
    <div className="relative w-full">
      <Input {...props} value={value} type={revealed ? "text" : "password"} className={cn("pr-16", className)} />
      <div className="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-0.5">
        <button type="button" tabIndex={-1} aria-label={revealed ? "Hide" : "Reveal"}
          className="p-1 rounded text-muted-foreground hover:bg-secondary/50"
          onClick={() => setRevealed(v => !v)}>
          {revealed ? <EyeOff size={16} /> : <Eye size={16} />}
        </button>
        <button type="button" tabIndex={-1} aria-label="Copy" disabled={!stringValue}
